...
import com.brain.declaration.config.KafkaProperties;
...
class DeclarationKafkaResourceIT {
    ...
    void setup() {
        ...
        DeclarationKafkaResource kafkaResource = new DeclarationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/declaration-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/declaration-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
